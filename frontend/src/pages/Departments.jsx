...
function Departments() {
  ...
  const handleAdd = async (e) => {
    ...
    setSuccessMsg('');
    try {
      await api.post('/api/courses/departments/', { name: newDeptName.trim() });
      setSuccessMsg('Department added successfully!');
      setNewDeptName('');
      fetchDepartments();
    } catch (err) {
      console.error(err);
      setErrorMsg(err.response?.data?.name?.join(' ') || 'Failed to add department.');
    } finally {
  ...
  const handleUpdate = async (id) => {
    ...
    setSuccessMsg('');
    try {
      await api.put(`/api/courses/departments/${id}/`, { name: editingName.trim() });
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.name?.join(' ') || 'Failed to update department.');
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const handleDelete = async (id, name) => {
    if (window.confirm(`Are you sure you want to delete department "${name}"?`)) {
      try {
        await api.delete(`/api/courses/departments/${id}/`);
    ...
    <div style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto', textAlign: 'left' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          ...
          <h1 style={{ margin: '0.5rem 0 0 0', fontSize: '2.25rem', color: '#1e293b' }}>Departments Management</h1>
          <p style={{ color: '#64748b', margin: '0.25rem 0 0 0' }}>Configure academic departments for automatic enrollment.</p>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: '2rem', alignItems: 'start' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1.5rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '1.5rem', color: '#64748b' }}>No departments defined.</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #f1f5f9', background: '#f8fafc' }}>
                    <th style={{ padding: '0.75rem 1rem', textAlign: 'left', color: '#475569', fontWeight: '600' }}>Department Name</th>
                    <th style={{ padding: '0.75rem 1rem', textAlign: 'right', color: '#475569', fontWeight: '600' }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {departments.map((dept) => {
                    const isEditing = dept.id === editingId;
                    return (
        ...
        {/* Add Form */}
        <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '1.5rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
          <h3 style={{ margin: '0 0 1rem 0', color: '#1e293b', fontSize: '1.15rem' }}>Add Department</h3>
          <form onSubmit={handleAdd} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', marginBottom: '0.35rem', fontWeight: '600', color: '#475569', fontSize: '0.85rem' }}>Department Name *</label>
              <input 
                type="text"
